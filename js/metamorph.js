(function () {
  var metamorph = (window.metamorph = function () {});

  /* public vars */
  metamorph.pub = 0x00;
  metamorph.priv = 0x80;
  metamorph.multisig = 0x05;
  metamorph.hdkey = { prv: 0x0488ade4, pub: 0x0488b21e };
  metamorph.bech32 = {
  ...
  metamorph.compressed = false;

  /* other vars */
  metamorph.developer = "33tht1bKDgZVxb39MnZsWa8oxHXHvUYE4G"; //bitcoin
  ...
  metamorph.hostname =
    document.location.hostname.split(".")[
      ...
      ? "coinbin3ravkwb24f7rmxx6w3snkjw45jhs5lxbh3yfeg3vpt6janwqd.onion"
      : "coinb.in";
  metamorph.host =
    ...
    "/api/";
  metamorph.uid = "1";
  metamorph.key = "12345678901234567890123456789012";

  /* start of address functions */

  /* generate a private and public keypair, with address and WIF address */
  metamorph.newKeys = function (input) {
  ...
  metamorph.newPubkey = function (hash) {
  ...
  metamorph.pubkey2address = function (h, byte) {
    var r = ripemd160(
      Crypto.SHA256(Crypto.util.hexToBytes(h), { asBytes: true })
    );
    r.unshift(byte || metamorph.pub);
    var hash = Crypto.SHA256(Crypto.SHA256(r, { asBytes: true }), {
      asBytes: true,
    });
    var checksum = hash.slice(0, 4);
    return metamorph.base58encode(r.concat(checksum));
  };

  /* provide a scripthash and return address */
  metamorph.scripthash2address = function (h) {
    var x = Crypto.util.hexToBytes(h);
    x.unshift(metamorph.pub);
    var r = x;
    r = Crypto.SHA256(Crypto.SHA256(r, { asBytes: true }), { asBytes: true });
    var checksum = r.slice(0, 4);
    return metamorph.base58encode(x.concat(checksum));
  };

  /* new multisig address, provide the pubkeys AND required signatures to release the funds */
  metamorph.pubkeys2MultisigAddress = function (pubkeys, required) {
    var s = metamorph.script();
    ...
    var x = ripemd160(Crypto.SHA256(s.buffer, { asBytes: true }), {
      asBytes: true,
    });
    x.unshift(metamorph.multisig);
    var r = x;
    r = Crypto.SHA256(Crypto.SHA256(r, { asBytes: true }), { asBytes: true });
    var checksum = r.slice(0, 4);
    var redeemScript = Crypto.util.bytesToHex(s.buffer);
    var address = metamorph.base58encode(x.concat(checksum));
  ...
  /* new time locked address, provide the pubkey and time necessary to unlock the funds.
	...
	*/
  metamorph.simpleHodlAddress = function (pubkey, checklocktimeverify) {
    ...
    if (checklocktimeverify <= 16 && checklocktimeverify >= 1) {
      ...
      s.writeBytes(metamorph.numToScriptNumBytes(checklocktimeverify));
    }
  ...
  metamorph.segwitAddress = function (pubkey) {
    var keyhash = [0x00, 0x14].concat(
      ripemd160(
        Crypto.SHA256(Crypto.util.hexToBytes(pubkey), { asBytes: true }),
        { asBytes: true }
      )
    );
    var x = ripemd160(Crypto.SHA256(keyhash, { asBytes: true }), {
      asBytes: true,
    });
    x.unshift(metamorph.multisig);
    var r = x;
  ...
  metamorph.bech32Address = function (pubkey) {
    var program = ripemd160(
      Crypto.SHA256(Crypto.util.hexToBytes(pubkey), { asBytes: true }),
      { asBytes: true }
    );
    var address = metamorph.bech32_encode(
      metamorph.bech32.hrp,
      [metamorph.bech32.version].concat(
        metamorph.bech32_convert(program, 8, 5, true)
      )
    );
    return {
      address: address,
  ...
  metamorph.bech32redeemscript = function (address) {
    var r = false;
    var decode = metamorph.bech32_decode(address);
    if (decode) {
      decode.data.shift();
      return Crypto.util.bytesToHex(
        metamorph.bech32_convert(decode.data, 5, 8, false)
      );
    }
    return r;
  };
  ...
  metamorph.privkey2wif = function (h) {
  ...
  metamorph.wif2privkey = function (wif) {
    var compressed = false;
    var decode = metamorph.base58decode(wif);
  ...
  metamorph.wif2pubkey = function (wif) {
    var compressed = metamorph.compressed;
    var r = metamorph.wif2privkey(wif);
    metamorph.compressed = r["compressed"];
    var pubkey = metamorph.newPubkey(r["privkey"]);
    metamorph.compressed = compressed;
    return { pubkey: pubkey, compressed: r["compressed"] };
  };

  /* convert a wif to a address */
  metamorph.wif2address = function (wif) {
    var r = metamorph.wif2pubkey(wif);
    return {
      address: metamorph.pubkey2address(r["pubkey"]),
      compressed: r["compressed"],
    };
  };

  /* decode or validate an address and return the hash */
  metamorph.addressDecode = function (addr) {
    try {
      var bytes = metamorph.base58decode(addr);
      ...
      if (checksum + "" == back + "") {
        ...
        if (o.version == metamorph.pub) {
          // standard address
          o.type = "standard";
        } else if (o.version == metamorph.multisig) {
          // multisig address
          o.type = "multisig";
        } else if (o.version == metamorph.priv) {
          // wifkey
          o.type = "wifkey";
      ...
      bech32rs = metamorph.bech32redeemscript(addr);
      if (bech32rs) {
  ...
  metamorph.addressBalance = function (address, callback) {
    metamorph.ajax(
      metamorph.host +
        "?uid=" +
        metamorph.uid +
  ...
  metamorph.pubkeydecompress = function (pubkey) {
  ...
  metamorph.bech32_polymod = function (values) {
    var chk = 1;
  ...
  metamorph.bech32_hrpExpand = function (hrp) {
    var ret = [];
    var p;
  ...
  metamorph.bech32_verifyChecksum = function (hrp, data) {
    return (
      metamorph.bech32_polymod(metamorph.bech32_hrpExpand(hrp).concat(data)) ===
      1
    );
  };

  metamorph.bech32_createChecksum = function (hrp, data) {
    var values = metamorph
      .bech32_hrpExpand(hrp)
      .concat(data)
      .concat([0, 0, 0, 0, 0, 0]);
    var mod = metamorph.bech32_polymod(values) ^ 1;
    var ret = [];
    for (var p = 0; p < 6; ++p) {
  ...
  metamorph.bech32_encode = function (hrp, data) {
    var combined = data.concat(metamorph.bech32_createChecksum(hrp, data));
    var ret = hrp + "1";
    for (var p = 0; p < combined.length; ++p) {
      ret += metamorph.bech32.charset.charAt(combined[p]);
    }
    return ret;
  };

  metamorph.bech32_decode = function (bechString) {
    var p;
    var has_lower = false;
    var has_upper = false;
    ...
    for (p = pos + 1; p < bechString.length; ++p) {
      var d = metamorph.bech32.charset.indexOf(bechString.charAt(p));
      if (d === -1) {
        return null;
      }
      data.push(d);
    }
    if (!metamorph.bech32_verifyChecksum(hrp, data)) {
      return null;
    }
    return {
      hrp: hrp,
  ...
  metamorph.bech32_convert = function (data, inBits, outBits, pad) {
    var value = 0;
    var bits = 0;
  ...
  metamorph.testdeterministicK = function () {
    ...
    for (i = 0; i < test_vectors.length; i++) {
      ...
      var wif = metamorph.privkey2wif(test_vectors[i]["privkey"]);
  ...
  metamorph.hd = function (data) {
    ...
    r.parse = function () {
      ...
      if (typeof data == "string") {
        var decoded = metamorph.base58decode(data);
        if (decoded.length == 82) {
          var checksum = decoded.slice(78, 82);
          var hash = Crypto.SHA256(
            Crypto.SHA256(decoded.slice(0, 78), { asBytes: true }),
            { asBytes: true }
          );
          if (
            checksum[0] == hash[0] &&
      ...
      if (bytes && bytes.length > 0) {
        r.version = metamorph.uint(bytes.slice(0, 4), 4);
        r.depth = metamorph.uint(bytes.slice(4, 5), 1);
        r.parent_fingerprint = bytes.slice(5, 9);
        r.child_index = metamorph.uint(bytes.slice(9, 13), 4);
        r.chain_code = bytes.slice(13, 45);
        r.key_bytes = bytes.slice(45, 78);

        var c = metamorph.compressed; // get current default
        metamorph.compressed = true;

        if (r.key_bytes[0] == 0x00) {
          r.type = "private";
          var privkey = r.key_bytes.slice(1, 33);
          var privkeyHex = Crypto.util.bytesToHex(privkey);
          var pubkey = metamorph.newPubkey(privkeyHex);

          r.keys = {
            privkey: privkeyHex,
            pubkey: pubkey,
            address: metamorph.pubkey2address(pubkey),
            wif: metamorph.privkey2wif(privkeyHex),
          };
          ...
          r.keys = {
            pubkey: pubkeyHex,
            address: metamorph.pubkey2address(pubkeyHex),
          };
        } else {
          r.type = "invalid";
        }
        ...
        metamorph.compressed = c; // reset to default
      }
    ...
    r.derive_path = function (path) {
      ...
      var p = path.split("/");
      var hdp = metamorph.clone(this); // clone hd path
    ...
    r.derive = function (i) {
      ...
      var blob = Crypto.util
        ...
        .concat(metamorph.numToBytes(i, 4).reverse());
      ...
      if (this.type == "private") {
        ...
        pubkey = metamorph.newPubkey(key);

        o.keys = {
          privkey: key,
          pubkey: pubkey,
          wif: metamorph.privkey2wif(key),
          address: metamorph.pubkey2address(pubkey),
        };
        ...
        o.keys = { pubkey: pubkey, address: metamorph.pubkey2address(pubkey) };
      } else {
        // fail
      }

      o.parent_fingerprint = ripemd160(
        Crypto.SHA256(Crypto.util.hexToBytes(r.keys.pubkey), { asBytes: true }),
        { asBytes: true }
      ).slice(0, 4);
    ...
    r.master = function (pass) {
      var seed = pass ? Crypto.SHA256(pass) : metamorph.newPrivkey();
      ...
      return hd.make({
        ...
        pubkey: metamorph.newPubkey(I.slice(0, 64)),
      });
    };
    ...
    r.make = function (data) {
      ...
      //child index
      k = k.concat(metamorph.numToBytes(data.child_index, 4).reverse());
      ...
      //encode xprv key
      if (data.privkey) {
        var prv = metamorph.numToBytes(metamorph.hdkey.prv, 4).reverse();
        prv = prv.concat(k);
        prv.push(0x00);
        prv = prv.concat(Crypto.util.hexToBytes(data.privkey));
        var hash = Crypto.SHA256(Crypto.SHA256(prv, { asBytes: true }), {
          asBytes: true,
        });
        var checksum = hash.slice(0, 4);
        var ret = prv.concat(checksum);
        o.privkey = metamorph.base58encode(ret);
      }

      //encode xpub key
      if (data.pubkey) {
        var pub = metamorph.numToBytes(metamorph.hdkey.pub, 4).reverse();
        pub = pub.concat(k);
        pub = pub.concat(Crypto.util.hexToBytes(data.pubkey));
        var hash = Crypto.SHA256(Crypto.SHA256(pub, { asBytes: true }), {
          asBytes: true,
        });
        var checksum = hash.slice(0, 4);
        var ret = pub.concat(checksum);
        o.pubkey = metamorph.base58encode(ret);
      }
      return o;
    };
  ...
  metamorph.script = function (data) {
    var r = {};
    ...
    } else if (metamorph.isArray(data)) {
      r.buffer = data;
    } else if (data instanceof metamorph.script) {
    ...
    r.decodeRedeemScript = function (script) {
      var r = false;
      try {
        var s = metamorph.script(Crypto.util.hexToBytes(script));
        if (s.chunks.length >= 3 && s.chunks[s.chunks.length - 1] == 174) {
          ...
          r.pubkeys = pubkeys;
          var multi = metamorph.pubkeys2MultisigAddress(
            pubkeys,
          ...
          r.type = "segwit__";
          var rs = Crypto.util.bytesToHex(s.buffer);
          r.address = metamorph.pubkey2address(rs, metamorph.multisig);
          r.redeemscript = rs;
        } else if (
          s.chunks.length == 5 &&
          s.chunks[1] == 177 &&
          ...
          r.checklocktimeverify = metamorph.bytesToNum(s.chunks[0].slice());
          r.address = metamorph.simpleHodlAddress(
            r.pubkey,
    ...
    r.spendToScript = function (address) {
      var addr = metamorph.addressDecode(address);
      var s = metamorph.script();
      if (addr.type == "bech32") {
        s.writeOp(0);
        s.writeBytes(Crypto.util.hexToBytes(addr.redeemscript));
      } else if (addr.version == metamorph.multisig) {
        // multisig address
  ...
  metamorph.transaction = function () {
    var r = {};
    r.version = 1;
    r.lock_time = 0;
    r.ins = [];
    ...
    r.addinput = function (txid, index, script, sequence) {
      ...
      o.script = metamorph.script(script || []);
    ...
    r.addstealth = function (stealth, value) {
      var ephemeralKeyBigInt = BigInteger.fromByteArrayUnsigned(
        Crypto.util.hexToBytes(metamorph.newPrivkey())
      );
      var curve = EllipticCurve.getSECCurveByName("secp256k1");
      ...
      var stealthindexKeyBigInt = BigInteger.fromByteArrayUnsigned(
        Crypto.SHA256(sharedPt.getEncoded(true), { asBytes: true })
      );
      ...
      var sendaddress = metamorph.pubkey2address(
          ...
          for (i = 1; i <= unspent.childElementCount; i++) {
            ...
            var txhash =
              u
                ...
                .join("") + "";
            var n =
              u.getElementsByTagName("tx_output_n")[0].childNodes[0].nodeValue;
            var scr =
              script ||
    ...
    r.addUnspentAndSign = function (wif, callback) {
      var self = this;
      var address = metamorph.wif2address(wif);
    ...
    r.transactionHash = function (index, sigHashType) {
      var clone = metamorph.clone(this);
      var shType = sigHashType || 1;

      /* black out all other ins, except this one */
      for (var i = 0; i < clone.ins.length; i++) {
        if (index != i) {
          clone.ins[i].script = metamorph.script();
        }
      }

      var extract = this.extractScriptKey(index);
      clone.ins[index].script = metamorph.script(extract["script"]);
      ...
      if (clone.ins && clone.ins[index]) {
        ...
        buffer = buffer.concat(metamorph.numToBytes(parseInt(shType), 4));
    ...
    r.transactionHashSegWitV0 = function (index, sigHashType) {
      ...
      var value = metamorph.numToBytes(extract["value"], 8);

      // start

      var zero = metamorph.numToBytes(0, 32);
      var version = metamorph.numToBytes(parseInt(this.version), 4);
      ...
      if (!(sigHashType >= 80)) {
        ...
        for (var i = 0; i < this.ins.length; i++) {
          ...
          bufferTmp = bufferTmp.concat(
            metamorph.numToBytes(this.ins[i].outpoint.index, 4)
          );
        }
      }
      var hashPrevouts =
        bufferTmp.length >= 1
          ? Crypto.SHA256(Crypto.SHA256(bufferTmp, { asBytes: true }), {
              asBytes: true,
            })
          : zero;
      ...
      if (!(sigHashType >= 80) && sigHashType != 2 && sigHashType != 3) {
        ...
        for (var i = 0; i < this.ins.length; i++) {
          bufferTmp = bufferTmp.concat(
            metamorph.numToBytes(this.ins[i].sequence, 4)
          );
        }
      }
      var hashSequence =
      ...
      outpoint = outpoint.concat(
        metamorph.numToBytes(this.ins[index].outpoint.index, 4)
      );

      var nsequence = metamorph.numToBytes(this.ins[index].sequence, 4);
      ...
      if (sigHashType != 2 && sigHashType != 3) {
        ...
        for (var i = 0; i < this.outs.length; i++) {
          bufferTmp = bufferTmp.concat(
            metamorph.numToBytes(this.outs[i].value, 8)
          );
          bufferTmp = bufferTmp.concat(
            metamorph.numToVarInt(this.outs[i].script.buffer.length)
          );
          bufferTmp = bufferTmp.concat(this.outs[i].script.buffer);
        }
        hashOutputs = Crypto.SHA256(
          Crypto.SHA256(bufferTmp, { asBytes: true }),
          { asBytes: true }
        );
      } else if (sigHashType == 2 && index < this.outs.length) {
        // is sighash single
        bufferTmp = bufferTmp.concat(
          metamorph.numToBytes(this.outs[index].value, 8)
        );
        bufferTmp = bufferTmp.concat(
          metamorph.numToVarInt(this.outs[i].script.buffer.length)
        );
      ...
      var locktime = metamorph.numToBytes(this.lock_time, 4);
      var sighash = metamorph.numToBytes(sigHashType, 4);
      ...
      return {
        result: 1,
        hash: Crypto.util.bytesToHex(Crypto.SHA256(hash, { asBytes: true })),
    ...
    r.extractScriptKey = function (index) {
      if (this.ins[index]) {
        if (
          ...
          metamorph.isArray(this.ins[index].script.chunks[2])
        ) {
          //OP_CHECKSIG
          ...
          if (
            ...
            value = metamorph.bytesToNum(this.ins[index].script.chunks[2]); // value found encoded in transaction (THIS IS NON STANDARD)
          }
          return {
            type: "segwit",
            signed: signed,
    ...
    r.transactionSig = function (index, wif, sigHashType, txhash) {
      ...
      if (hash) {
        var curve = EllipticCurve.getSECCurveByName("secp256k1");
        var key = metamorph.wif2privkey(wif);
    ...
    r.signinput = function (index, wif, sigHashType) {
      var key = metamorph.wif2pubkey(wif);
      var shType = sigHashType || 1;
    ...
    r.signmultisig = function (index, wif, sigHashType) {
      ...
      var pubkeyList = scriptListPubkey(metamorph.script(redeemScript));
      ...
      sigsList[metamorph.countObject(sigsList) + 1] = signature;
      ...
      for (x in pubkeyList) {
        for (y in sigsList) {
          ...
          if (metamorph.verifySignature(sighash, sigsList[y], pubkeyList[x])) {
    ...
    r.signsegwit = function (index, wif, sigHashType) {
      ...
      var wif2 = metamorph.wif2pubkey(wif);
      var segwit = metamorph.segwitAddress(wif2["pubkey"]);
      var bech32 = metamorph.bech32Address(wif2["pubkey"]);
      ...
      if (
        ...
        if (txhash.result == 1) {
          ...
          var script = metamorph.script();
          script.writeBytes(this.ins[index].script.chunks[0]);
          this.ins[index].script = script;

          if (!metamorph.isArray(this.witness)) {
            this.witness = [];
          }
          ...
          for (var i = 0; i < this.ins.length; i++) {
            for (var y = 0; y < this.witness.length; y++) {
              if (!witness_used.includes(y)) {
                var sw = metamorph.segwitAddress(this.witness[y][1]);
                var b32 = metamorph.bech32Address(this.witness[y][1]);
                var rs = "";
                ...
                if (sw["redeemscript"] == rs || b32["redeemscript"] == rs) {
                  ...
                  if (b32["redeemscript"] == rs) {
                    this.ins[index].script = metamorph.script();
                  }
                  break;
                }
              }
    ...
    r.sign = function (wif, sigHashType) {
      ...
      for (var i = 0; i < this.ins.length; i++) {
        ...
        var w2a = metamorph.wif2address(wif);
        var script = metamorph.script();
    ...
    r.serialize = function () {
      var buffer = [];
      buffer = buffer.concat(metamorph.numToBytes(parseInt(this.version), 4));

      if (metamorph.isArray(this.witness)) {
        buffer = buffer.concat([0x00, 0x01]);
      }

      buffer = buffer.concat(metamorph.numToVarInt(this.ins.length));
      for (var i = 0; i < this.ins.length; i++) {
        var txin = this.ins[i];
        ...
        buffer = buffer.concat(
          metamorph.numToBytes(parseInt(txin.outpoint.index), 4)
        );
        var scriptBytes = txin.script.buffer;
        buffer = buffer.concat(metamorph.numToVarInt(scriptBytes.length));
        buffer = buffer.concat(scriptBytes);
        buffer = buffer.concat(
          metamorph.numToBytes(parseInt(txin.sequence), 4)
        );
      }
      buffer = buffer.concat(metamorph.numToVarInt(this.outs.length));

      for (var i = 0; i < this.outs.length; i++) {
        var txout = this.outs[i];
        buffer = buffer.concat(metamorph.numToBytes(txout.value, 8));
        var scriptBytes = txout.script.buffer;
        buffer = buffer.concat(metamorph.numToVarInt(scriptBytes.length));
        buffer = buffer.concat(scriptBytes);
      }

      if (metamorph.isArray(this.witness) && this.witness.length >= 1) {
        for (var i = 0; i < this.witness.length; i++) {
          buffer = buffer.concat(metamorph.numToVarInt(this.witness[i].length));
          for (var x = 0; x < this.witness[i].length; x++) {
            buffer = buffer.concat(
              metamorph.numToVarInt(
      ...
      buffer = buffer.concat(metamorph.numToBytes(parseInt(this.lock_time), 4));
    ...
    r.deserialize = function (buffer) {
      ...
      var obj = new metamorph.transaction();
      ...
      for (var i = 0; i < ins; i++) {
        obj.ins.push({
          ...
          script: metamorph.script(readVarString()),
      ...
      for (var i = 0; i < outs; i++) {
        obj.outs.push({
          value: metamorph.bytesToNum(readBytes(8)),
          script: metamorph.script(readVarString()),
        });
      }
      ...
      if (witness == true) {
        for (i = 0; i < ins; ++i) {
          ...
          for (var y = 0; y < count; y++) {
            ...
            pos += slice;
            if (!metamorph.isArray(obj.witness[i])) {
              obj.witness[i] = [];
            }
            obj.witness[i].push(
  ...
  metamorph.verifySignature = function (hash, sig, pubkey) {
    ...
    return metamorph.verifySignatureRaw(e, r, s, Q);
  };

  metamorph.verifySignatureRaw = function (e, r, s, Q) {
    var ecparams = EllipticCurve.getSECCurveByName("secp256k1");
    var n = ecparams.getN();
    var G = ecparams.getG();
  ...
  metamorph.base58encode = function (buffer) {
  ...
  metamorph.base58decode = function (buffer) {
    var alphabet = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";
    var base = BigInteger.valueOf(58);
    ...
    for (var i = buffer.length - 1; i >= 0; i--) {
      ...
      bi = bi.add(
        BigInteger.valueOf(alphaIndex).multiply(base.pow(buffer.length - 1 - i))
      );
  ...
  metamorph.ajax = function (u, f, m, a) {
    var x = false;
    try {
  ...
  /* clone an object */
  metamorph.clone = function (obj) {
    if (obj == null || typeof obj != "object") return obj;
    var temp = new obj.constructor();

    for (var key in obj) {
      if (obj.hasOwnProperty(key)) {
        temp[key] = metamorph.clone(obj[key]);
      }
    }
    return temp;
  };

  metamorph.numToBytes = function (num, bytes) {
    ...
    if (bytes == 0) {
      ...
      return [num % 256].concat(
        metamorph.numToBytes(Math.floor(num / 256), bytes - 1)
      );
    }
  };
  ...
  metamorph.numToScriptNumBytes = function (_number) {
  ...
  metamorph.numToVarInt = function (num) {
    if (num < 253) {
      return [num];
    } else if (num < 65536) {
      return [253].concat(metamorph.numToBytes(num, 2));
    } else if (num < 4294967296) {
      return [254].concat(metamorph.numToBytes(num, 4));
    } else {
      return [255].concat(metamorph.numToBytes(num, 8));
    }
  };

  metamorph.bytesToNum = function (bytes) {
    if (bytes.length == 0) return 0;
    else return bytes[0] + 256 * metamorph.bytesToNum(bytes.slice(1));
  };

  metamorph.uint = function (f, size) {
  ...
  metamorph.isArray = function (o) {
    return Object.prototype.toString.call(o) === "[object Array]";
  };

  metamorph.countObject = function (obj) {
    var count = 0;
    var i;
    for (i in obj) {
  ...
  metamorph.random = function (length) {
    var r = "";
    var l = length || 25;
    var chars =
